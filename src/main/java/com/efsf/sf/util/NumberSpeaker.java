...
public class NumberSpeaker {
    ...
    public static String speakNumber(int value, int currency) {
        return speakNumber((long) value, currency);
    }
    
    public static String speakNumber(double value) {
       
        int decimal = (int) value;
        double fractional = (value - decimal)*100; 
        int fract=(int) Math.round(fractional);
        
        return speakNumber(decimal, 1)
                +" "+speakNumber(fract, 2);
    }
    ...
    private static String getCurrency(int liczba, int type)
    {
        if (liczba == 1)
            return type==1? ZL[0] : GR[0];

        int para = liczba % 100;
        if (para >= 10 && para < 20)
            return type==1? ZL[2] : GR[2];

        int jednosc = liczba % 10;
        if (jednosc >= 2 && jednosc <= 4)
            return type==1? ZL[1] : GR[1];
